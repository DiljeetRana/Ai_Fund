...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onNewChat }) => {
  return (
    <div
      className={`fixed lg:relative w-72 h-full bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 z-50 ${
    ...
    >
      ...
      <div className="p-5">
        <button
          onClick={onNewChat}
          className="w-full py-3.5 px-5 bg-primary hover:bg-primary-dark text-white rounded-xl font-semibold flex items-center justify-center gap-2 transition-all hover:shadow-lg hover:-translate-y-0.5"
        >
...
const MenuItem: React.FC<MenuItemProps> = ({ icon, label, active }) => {
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all my-1 ${
        active
